...
def convertToTime(time):
    try:
        ...
        messagebox.showerror("Error in Syntax", "Enter Hours in HH:MM format!!!")
        block = SelectBlock(root,)
    ...
    def submit(self):
        if self.seat == '':
            ...
            if result == 'yes':
                ...
                conn.commit()
                confirm = confirmation(root, self.studId)
...
class confirmation:
    def __init__(self, root, studId):
        ...
        self.query = f"SELECT * FROM Booking WHERE StudentId LIKE '{self.studId}'"
        cursor.execute(self.query)
        result = cursor.fetchone()
        self.details = [i for i in result]
        ...
        self.reg_no_label.place(x = 50, y = 225)
        self.reg_no_label_text = Label(self.confirm_frame, text = self.details[4], font = fonts, bg = 'dark turquoise', fg = 'navy', width =5)
        ...
        self.ctnum_label_text = Label(self.confirm_frame, text = self.details[3], font = fonts, bg = 'dark turquoise', fg = 'navy', width = 5)
        ...
        self.date_label_text = Label(self.confirm_frame, text = self.details[0], font = fonts, bg = 'dark turquoise', fg = 'navy', width = 10)
        ...
        s = self.details[1] +  '-' + self.details[2]
    ...
    try:
        def cancel(self):
            ...
            if r == 'yes':
                self.query = f"DELETE FROM Booking WHERE StudentId LIKE '{self.studId}'"
                cursor.execute(self.query)
                self.deleteinlab()
        ...
        def deleteinlab(self):
            if self.details[3][0] == 'D':
                    self.query = f"DELETE FROM DBlockVacancy WHERE CompId LIKE '{self.details[3]}'"
            else:
                self.query = f"DELETE FROM LibVacancy WHERE CompId LIKE '{self.details[3]}'"
            cursor.execute(self.query)
